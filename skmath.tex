...
\begin{document}
  ...
  \changes{0.6}{Uses \cs{mleft} and \cs{mright} from \pkg{mleftright} for better spacing}
...
\PassOptionsToPackage{intlimits,sumlimits,namelimits}{amsmath}
\RequirePackage{amssymb,mathtools,mleftright,xfrac}
\end{MacroCode}
...
\cs_new_nopar:Npn\__skmath_parens:n#1{
  \__skmath_if_novalue_or_empty:nF{#1}{\mleft(#1\mright)}
}
\end{MacroCode}
  \end{macro*}
...
\AtBeginDocument{
  \DeclareDocumentCommand\E{m}{%
    \ensuremath{\operatorname{E}\mleft[#1\mright]}%
  }
}
\end{MacroCode}
  \end{macro}
...
\DeclareDocumentCommand\var{m}{%
  \ensuremath{\operatorname{Var}\mleft(#1\mright)}%
}
\end{MacroCode}
  \end{macro}
  \begin{macro}{\cov}
...
\DeclareDocumentCommand\cov{mm}{%
  \ensuremath{\operatorname{Cov}\mleft(#1,#2\mright)}%
}
\end{MacroCode}
  \end{macro}
...
\cs_new_nopar:Npn\__skmath_minmax_backend:nnnn#1#2#3#4{%
  ...
  \IfBooleanTF{#2}{#4}{\mleft\{#4\mright\}}
}
\end{MacroCode}
  \end{macro*}
  \begin{macro}{\min}
